...
struct UserData {
  ...
  // a list of all the element to be be diagnosed
  std::vector<GstElement*> elements_to_diagnose;
};
...
cxxopts::Options build_arg_parser()
{
  ...
  ("i, input", "Set the input source", cxxopts::value<std::string>()->default_value(std::string("/dev/video0")))
