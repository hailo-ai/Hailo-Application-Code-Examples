...
====================
This example performs image classification using a Hailo-8, Hailo-8L, or Hailo-10H device.
It receives a HEF and images/video/camera as input, and returns the image/video with the top-1 predicted class overlaid.

![output example](./classifier.jpg)
...
2. Compile the project on the development machine  
    ...
    ./build.sh
    ```
    This creates the directory hierarchy build/x86_64 and compile an executable file called classifier

3. Run the example:

    ```shell script
    ./build/x86_64/classifier --net <hef_path> --input <image_or_video_or_camera_path>
    ```
