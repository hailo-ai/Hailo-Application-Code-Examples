...
class PoseEstPostProcessing:
    ...
    def postprocess_and_visualize(
        ...
        output_image = visualize_pose_estimation_result(results, image)
    ...
    def extract_pose_estimation_results(
        ...
        decoded_boxes, decoded_kpts = decoder(raw_boxes,
                                              kpts, strides,
...
def visualize_pose_estimation_result(
    results: dict, img: Image.Image, *, detection_threshold: float = 0.5,
...
def log_layer_info(
    layers: list, log: logging.Logger, layer_type: str = 'Input'
) -> None:
    """
    Log information about layers.

    Args:
        layers (List): List of layer objects, each with `name` and `shape` attributes.
        log (logging.Logger): Logger object for logging the information.
        layer_type (str): Type of the layer to log (e.g., 'Input' or 'Output').

    Returns:
        None
    """
    for layer_info in layers:
        log.info('{} layer: {} {}'.format(layer_type, layer_info.name, layer_info.shape))


def preprocess(image: Image.Image, model_w: int, model_h: int) -> Image.Image:
    """
...
def decoder(
    raw_boxes: np.ndarray, raw_kpts: np.ndarray, strides: list[int], 
...
def non_max_suppression(
    prediction: np.ndarray, conf_thres: float = 0.1, iou_thres: float = 0.45,
