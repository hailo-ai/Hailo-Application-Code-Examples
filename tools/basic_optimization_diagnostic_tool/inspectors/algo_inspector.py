...
from hailo_model_optimization.acceleras.utils.acceleras_definitions import (
    ModelOptimizationCommand, MOConfigCommand, PostQuantizationFeature)
...
class PostQuantAlgoInspector(BaseInspector):
    ...
    def _run(self):
        ...
        features_summary = {feature.value: {} for feature in PostQuantizationFeature}
        ...
        for feature, info in features_summary.items():
            self._logger.info(f"{feature} is {'enabled' if info['enabled'] else 'disabled'}")
            if info['enabled']:
                self._logger.debug(f"{feature}: {info['kwargs']}")
        if implicit:
